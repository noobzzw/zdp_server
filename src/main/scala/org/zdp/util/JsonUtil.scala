...
/**
  ...
  */
object JsonUtil {
  case object TimestampSerializer extends CustomSerializer[java.sql.Timestamp](_ => ( {
    case _ => null
  }, {
    case ts: Timestamp =>JLong(ts.getTime)
  })
  )
  // json4s's default date format
  implicit private val dateFormats = DefaultFormats
